...
- **Label and overview performance budget:** scene labels project to whole CSS pixels with a 0.75 px deadband and avoid continuously repainting backdrop blur over WebGL. Distant views unmount hidden moon/minor-body/spacecraft DOM labels, moon meshes and local orbits, plus unselected mission-trail geometry. When the camera is both distant and aimed at the system center, DPR is capped at 1.2, composer MSAA is disabled, Bloom is reduced, and minor-body orbits are culled; focused distant missions and selected targets retain detail
