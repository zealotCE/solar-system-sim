...
具有未来观测台视觉的交互式三维太阳系：八大行星与冥王星、26 颗天然卫星、9 个重点小天体/彗星、19 个著名人类航天器、主小行星带与柯伊伯带、真实星历驱动的位置与时间机器（1950–2050），支持严格真实比例和纯中文 / 双语 / 纯英文界面。位置与时间来自打包的 JPL / NASA 公开数据及明确标注的代表性轨道，用于模型展示，不代表实时测量。
...
- **严格真实比例模式**：一键切换，半径与轨道共用同一 km→场景映射，无任何物理体积放大（太阳半径约为地球轨道的 1/215）；航天器实体也按公开最大展开跨度换算。远距半透明识别层会分析 GLB 表面积分布，以 64–84 px 的可辨主体为目标，而不是用细长天线或磁强计杆的总包围盒决定倍率；全部外伸结构仍限制在 156 px 内。识别层在真实主体达到 18–40 px 时连续收敛，并于 34–40 px 滞回区间交给米级物理模型，加载失败则保留程序化回退。超近裁剪面、目标主体锚定标签和局部轨迹弧支持从太阳系全景连续缩放到航天器表面尺度
...
- **罗曼望远镜发射更新**：南希·格蕾丝·罗曼空间望远镜按 NASA 确认的 2026 年 8 月 30 日发射日期加入时间机器；发射前不会出现在物理场景，发射后沿明确标注为“示意而非实时测轨”的约 90 天日地 L2 转移逐步离开地球。艺术化地月系统沿用月球的 km→局部单位映射，L2 稳态位置约为月球轨道半径的 3.9 倍；刚发射的 Roman 在实际越过月球距离前仍会显示在月球轨道内。档案收录提前约九个月发射、2.4 米主镜、百倍哈勃视场和日冕仪技术验证，并使用 NASA 完工构型高精度模型
...
- **近地任务距离分级**：太阳系远景把哈勃、ISS、天宫、Webb 和已发射的 Roman 合并为一个让出地球点击区的稳定标签簇，不挂载快速运动实体或局部轨道；聚合面板按中英文内容自适应列宽，极窄视口自动单列。代表性近地轨道投影半径达到 32 px（退场阈值 22 px）后才恢复细节。默认 1× 下约 90 分钟轨道的不可辨角速度压缩至最高 0.16 圈/秒，航天器仍连续运动；暂停或 0.01× 时使用精确模型相位
- **三档标签与远景性能预算**：标签按钮按「关 → 主要 → 全部」循环；“主要”保持距离分级与近地聚合，“全部”展开所有在当前日期真实存在的已命名自然/人造目标。远距 HTML 外围继续穿透场景，只有原生按钮标签胶囊接收点击；投影接近的远距航天器标签会低频自动分离，因此可直接选择旅行者 2 号、卫星和小天体而不会用整块容器遮挡地球。普通标签同帧刷新目标与相机矩阵，以 1/8 px 子像素步进和 0.1 px 滞回平滑移动；选中目标使用精确连续投影，整个胶囊作为单一合成层平移，避免文字基线上下跳动。远景卸载隐藏 DOM、实体、局部轨道和非选中航迹。摄像机远离且朝向太阳系中心时，DPR 上限降至 1.2、关闭合成器 MSAA、降低 Bloom 并裁剪小天体轨道
- **轨道层级与屏幕空间 LOD**：八大行星轨道使用更亮、更宽的主骨架线；冥王星、阿罗科斯等矮行星/小天体及卫星保留较细较淡的密切轨道。闭合轨道按投影弦高在 128–16384 顶点间分级，Horizons 航迹按 overview/medium/focus 精度自适应细分；真实比例近距观察会把选中目标切换为穿过实体中心、按相机距离和局部速度取样的 256 段局部弧，地球/木星轨道器始终在母星局部坐标系取样，避免整圈轨道在极端放大后显露折角或切换时改变屏幕倾角。进入冥王星等行星卫星系统近景后会隐藏无关日心轨道，拉远越过滞回范围后自动恢复。柯伊伯带由 30–50 AU 的 1,800 个确定性统计点构成，太阳系远景降至 900 点
- **权威与自建 3D 形状模型**：航天器与谷神星、灶神星、贝努使用 NASA 资源；67P、阿波菲斯和阿罗科斯新增 ESA Rosetta / NASA PDS 测绘形状模型。Roman 的 NASA 完工构型源模型由 29.66 MB 优化为约 3.79 MB，保留材质、法线与完整展开结构；Europa Clipper 的 34.05 MiB 原始文件优化为约 2.6 MiB。因未找到可再分发的天宫官方工程网格，项目依据 CMSA/CNSA 尺寸与构型资料自建约 122 KiB 的 2026 三舱 T 字组合体，包含问天/梦天四副大型柔性太阳翼、天舟十号及神舟二十三号。档案提供自动/当前/规划三档构型：自动时间线从 2028-01-01 起显示约 130 KiB 的十字构型“规划推演”，但该阈值明确不是官方发射日期
...
项目具备接入赞助或广告的技术条件，但 NASA/JPL/PDS 数据、机构标识、CC 素材和第三方衍生纹理不能被同一个项目许可证一并重新授权。正式商业运营前需要建立逐文件素材台账，并优先替换或取得来源不完整、用途限制不够明确的纹理许可。完整风险、广告发布门槛和非背书要求见 [商业化与素材合规](docs/商业化与素材合规.md)；逐文件素材台账见 [THIRD_PARTY_ASSETS.md](THIRD_PARTY_ASSETS.md)，第三方署名与非背书声明见 [NOTICE](NOTICE)。
