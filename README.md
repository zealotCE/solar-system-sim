...
- **严格真实比例模式**：一键切换，半径与轨道共用同一 km→场景映射，无任何物理体积放大（太阳半径约为地球轨道的 1/215）；航天器实体也按公开最大展开跨度换算。远距时以 28–44 px 半透明识别模型定位，继续向内缩放会自动隐藏识别层并交还米级物理模型；超近裁剪面与局部轨迹基准支持从太阳系全景连续缩放到航天器表面尺度
