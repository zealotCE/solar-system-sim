  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        {MISSION_STORIES.map((item, index) => <button key={item.id} type="button" data-active={index === storyIndex} className="mission-story-tab" onClick={() => changeStory(index)}><span>{englishOnly ? item.titleEn : item.title}</span><small>{STORY_ENGLISH_NAMES[item.craftId]}</small></button>)}
      </div>
      ...
      <div className="mission-timeline" aria-label={pureChinese ? '事件时间线' : 'Event timeline'}>
        {story.events.map((item, index) => <button key={`${item.id}-${item.date}`} type="button" data-active={index === eventIndex} className="mission-event" onClick={() => setEventIndex(index)}><span className="mission-event-dot" /><span><small>{formatDate(item.date)}</small><strong>{englishOnly ? item.titleEn : item.title}</strong></span></button>)}
      </div>
      <article className="rounded-xl border border-cyan-200/10 bg-cyan-300/[0.035] p-3.5">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2"><button type="button" className="story-action" onClick={() => selectStoryEvent(story.id, event.id)}><Play className="size-3" />{pureChinese ? '回到当天 · 真实比例' : 'TRAVEL · TRUE SCALE'}</button><a className="story-source" href={event.sourceUrl} target="_blank" rel="noreferrer">{pureChinese ? '来源' : 'SOURCE'}<ExternalLink className="size-3" /></a></div>
      </article>
      ...
      <p className="flex items-center gap-1.5 text-[9px] text-slate-600"><MapPin className="size-3" />{pureChinese ? `聚焦目标：${focusTargetLabel(event.focusTargetId, languageMode)}` : `FOCUS TARGET: ${focusTargetLabel(event.focusTargetId, languageMode)}`}</p>
    </div>
  )
}
