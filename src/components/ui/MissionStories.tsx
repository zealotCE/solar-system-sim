  ...
  return (
    <div className="space-y-4">
      ...
      <article className="rounded-xl border border-cyan-200/10 bg-cyan-300/[0.035] p-3.5">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2"><button type="button" className="story-action" onClick={() => selectStoryEvent(story.id, event.id)}><Play className="size-3" />{pureChinese ? '回到当天并聚焦' : 'TRAVEL & FOCUS'}</button><a className="story-source" href={event.sourceUrl} target="_blank" rel="noreferrer">{pureChinese ? '来源' : 'SOURCE'}<ExternalLink className="size-3" /></a></div>
      </article>
